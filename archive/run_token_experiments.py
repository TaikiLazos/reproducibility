...
def train_and_evaluate(model, train_loader, val_loader, device, model_save_path, num_labels):
    ...
    for epoch in range(max_epochs):
        ...
        # Evaluate
        results = evaluate_tokens(model, val_loader, device)
        print(f"\nValidation Results:")
...
def evaluate_tokens(model, val_loader, device):
    model.eval()
    correct_tokens = 0  # Correct token predictions
    total_pred = 0     # Total tokens predicted as entities
    total_true = 0     # Total true entity tokens
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            for pred_seq, label_seq, mask in zip(predictions, batch['labels'], batch['attention_mask']):
                ...
                # Compare each token's prediction with its true label
                for pred, true in zip(pred_seq, label_seq):
                    if true == -100:  # Skip special tokens
                        continue
                        
                    if true != 0:  # True entity token
                        total_true += 1
                    
                    if pred != 0:  # Predicted entity token
                        total_pred += 1
                        if pred == true:  # Correct prediction
                            correct_tokens += 1
    
    # Calculate metrics
    precision = correct_tokens / total_pred if total_pred > 0 else 0
    recall = correct_tokens / total_true if total_true > 0 else 0
